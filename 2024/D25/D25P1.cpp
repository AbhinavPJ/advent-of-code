...
/*
   ...
   each grid has 5 numbers 1000 such grids
   O(n^2) trivial
   sort by first column,create an index mapping then use a segment tree to range update then point query
*/
...
signed main()
{
    ...
    for (int i = 0; i < lockheights.size(); i++)
    {
        for (int j = 0; j < keyheights.size(); j++)
        {
            ...
            for (int k = 0; k < 5; k++)
            {
                if (lockheights[i][k] + keyheights[j][k] > 7)
                {
                    good = false;
                    break;
                }
