 ...
 * created: 18:31:32 on 04-12-2025
 **/
#include <iostream>
#include <fstream>
#include <sstream>
...
void solve()
{
    ...
    while (getline(cin, s))
    {
        ...
        for (int i = l - 2; i >= 0; --i)
        {
            ...
            {
                if (dp[i + 1][j])
                    dp[i][j + 1] = max(dp[i][j + 1], dp[i + 1][j] + expo(10, j) * digit);
                dp[i][j + 1] = max(dp[i][j + 1], dp[i + 1][j + 1]);
            }
            dp[i][12] = max(dp[i][12], dp[i + 1][12]);
            mx2 = max(mx2, dp[i][12]);
        }
        ans += mx;
        ans2 += mx2;
...
signed main()
{
    ...
    cerr << "Time elapsed: " << elapsed.count() << " seconds" << endl;
    return 0;
}