 ...
 * created: 16:15:54 on 12-12-2025
 **/
#include <iostream>
#include <fstream>
#include <sstream>
...
int bfs(string s)
{
    // x represents all the choices we have to toggle
    if (s == target)
    {
        return 0;
    }
    // minimum steps,so bfs instead of dfs
...
int solveline(string s)
{
    ...
    while (ss >> word)
    {
        if (!word.empty() && word[0] == '{')
        {
            // initialize joltage
            int s = word.size();
            word = word.substr(1, s - 2);
            stringstream sp(word);
            string num;
            joltage.clear();
