 ...
 * created: 14:12:19 on 05-12-2025
 **/
#include <iostream>
#include <fstream>
#include <sstream>
...
void solve()
{
    ...
    while (getline(cin, s))
    {
        ...
        if (!flag)
        {
            ...
            int l = stol(s.substr(0, dashIndex));
            int r = stol(s.substr(dashIndex + 1));
            lr.pb({l, r});
        }
    }
    sort(all(lr));
    ...
    {
        points.pb(interval[0]);
        points.pb(interval[1]);
    ...
    {
        int x = stol(s);
        int less = lower_bound(all(points), x) - points.begin();
        if (less % 2 == 1)
        {
            ans += 1;
        }
