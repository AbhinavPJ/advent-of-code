 ...
 * created: 19:46:01 on 12-12-2025
 **/
#include <iostream>
#include <fstream>
#include <sstream>
...
*/
void dfs(int node, vvi &adj, vi &toposort, vector<int> &vis)
{
    vis[node] = 1;
    for (auto &it : adj[node])
        if (!vis[it])
            dfs(it, adj, toposort, vis);
...
void solve()
{
    ...
    while (getline(cin, s))
    {
        ...
        while (ss >> temp)
        {
            ...
            edges.pb({mp[word], mp[temp]});
        }
    }
    vvi adj(mp.size() + 1);
    vvi transpose(mp.size() + 1);
    for (auto &it : edges)
    {
        adj[it[0]].pb(it[1]);
        transpose[it[1]].pb(it[0]);
    }
    // toposort
    ...
    // we need to find number of paths from each node to out
    int special = mp["out"];
    vi dp(mp.size() + 1, 0);
    dp[special] = 1;
    for (auto &it : toposort)
    {
        for (auto &child : transpose[it])
        {
            dp[child] += dp[it];
        }
    }
    ...
    int ans1 = dp[mp["dac"]] * dp1[mp["fft"]] * dp2[mp["svr"]];
    cout << dp[mp["svr"]] << " " << ans1 << endl;
}
